...
void intersectGPU(int tbegin, int tend, int rend)
{
	...
	TIMED_CALL(cudaMemcpyAsync(d_tris[job], h_tris[job], tcount*sizeof(TriV), cudaMemcpyDefault, stream), s_triSendTime);
	TIMED_CALL(cudaMemcpyAsync(d_rays[job], h_rays[job], rend*sizeof(Ray), cudaMemcpyDefault, stream), s_raySendTime);

//	cudaDeviceSynchronize();
//	s_t.restart();
	kernel<<<numBlocks, BLOCKSIZE, BLOCKSIZE*sizeof(TriV), stream>>>(d_tris[job], tbegin, numTiles, lastTileSize, d_rays[job], rend, d_tmaxs[job], d_hits[job]);
//	cudaDeviceSynchronize();
//	s_kernelTime += s_t.msec();

	cudaEventRecord(jobEvent[job], stream);

	TIMED_CALL(cudaMemcpyAsync(h_tmaxs[job], d_tmaxs[job], rend*sizeof(float), cudaMemcpyDefault, stream), s_tmaxReadTime);
	TIMED_CALL(cudaMemcpyAsync(h_hits[job], d_hits[job], rend*sizeof(int), cudaMemcpyDefault, stream), s_hitReadTime);

	cudaStreamAddCallback(stream, updateKernelResults, (void*)job, 0);
