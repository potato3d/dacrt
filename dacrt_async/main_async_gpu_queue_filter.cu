...
void intersectGPU(int tbegin, int tend, Ray* rays, int rend)
{
	...
	std::copy(rays, rays + rend, h_rays[nextJob]);
	...
	cudaMemcpyAsync(g_rays.tmaxs, d_tmaxs, g_rays.count*sizeof(float), cudaMemcpyDefault, stream);

	++s_numIntersectCalls;
}

void intersectCPU(int tbegin, int tend, Ray* rays, int rend)
{
...
bool hitBox(const Box& box, const Ray& ray)
{
...
void filterRays(const Box& boxLeftTris, const Box& boxRightTris, Ray* rays, int rend, Ray* raysLeft, Ray* raysRight, int& rendLeft, int& rendRight)
{
	rendLeft = 0;
	rendRight = 0;
	for(int r = 0; r < rend; ++r)
	{
		...
		if(hitBox(boxLeftTris, ray))
		{
			raysLeft[rendLeft] = ray;
			++rendLeft;
		}
		if(hitBox(boxRightTris, ray))
		{
			raysRight[rendRight] = ray;
			++rendRight;
		}
	}
}
...
void trace(const Box& boxTris, const Box& boxCenters, int tbegin, int tend, Ray* rays, int rend)
{
#ifdef HIT_CPU
	if((tend-tbegin) < 8 || rend < 8) // todo: define better
	{
		TIMED_CALL(intersectCPU(tbegin, tend, rays, rend), s_intersectTime);
		return;
	}
	...
	if((tend-tbegin) < 8192 && rend < 32768)  // todo: define better
	{
		static timer __t;
		__t.restart();
		intersectGPU(tbegin, tend, rays, rend);
	...
	Ray* raysLeft = new Ray[rend];
	Ray* raysRight = new Ray[rend];
	int rendLeft, rendRight;
	TIMED_CALL(filterRays(boxLeftTris, boxRightTris, rays, rend, raysLeft, raysRight, rendLeft, rendRight), s_filterRayTime);

	if(rays[0].d[axis] >= 0)
	{
		delete[] rays;
		trace(boxLeftTris, boxLeftCenters, tbegin, tsplit, raysLeft, rendLeft);
		trace(boxRightTris, boxRightCenters, tsplit, tend, raysRight, rendRight);
	}
	else
	{
		delete[] rays;
...
void shadePixels(unsigned char* pixels)
{
	...
	for(int i = 0; i < g_rays.count; ++i)
	{
		...
		if(tmax < FLT_MAX)
		{
			pixels[ray.id*3+0] = 255;
			pixels[ray.id*3+1] = 255;
			pixels[ray.id*3+2] = 255;
...
void render(unsigned char* pixels)
{
	...
	Ray* rays = new Ray[g_rays.count];
	std::copy(g_rays.rays, g_rays.rays + g_rays.count, rays);

	t.restart();
	trace(g_tris.boxTris, g_tris.boxCenters, 0, g_tris.count, rays, g_rays.count);
