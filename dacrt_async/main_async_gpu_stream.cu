...
//	cudaEventSynchronize(kernelEvent);
	...
	TIMED_CALL(cudaMemcpyAsync(d_tris, g_tris.verts + tbegin, tcount*sizeof(TriV), cudaMemcpyDefault, stream), s_triSendTime);
	TIMED_CALL(cudaMemcpyAsync(d_rays, g_rays.rays, rend*sizeof(Ray), cudaMemcpyDefault, stream), s_raySendTime);
