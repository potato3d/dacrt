...
static double s_rayReadTime = 0.0;
...
static TriV g_paddingTri = {vec3(1000,1000,1000), vec3(1001,1000,1000), vec3(1000,1001,1000)};
static Ray g_paddingRay = {vec3(10000,10000,10000), vec3(0,0,-1), 0, 0};
...
__global__ void kernel(TriV* tris, int numTiles, int lastTileSize, Ray* rays, int numRays, int* hits)
{
...
void intersectGPU(int tbegin, int tend, int rend)
{
	...
	TIMED_CALL(cudaMemcpy(d_tris, g_tris.verts + tbegin, tcount*sizeof(TriV), cudaMemcpyDefault), s_triSendTime);
	TIMED_CALL(cudaMemcpy(d_rays, g_rays.rays, rend*sizeof(Ray), cudaMemcpyDefault), s_raySendTime);

	s_t.restart();
	kernel<<<numBlocks, BLOCKSIZE, BLOCKSIZE*sizeof(TriV)>>>(d_tris, numTiles, lastTileSize, d_rays, rend, d_hits);
	cudaDeviceSynchronize();
	s_kernelTime += s_t.msec();

	TIMED_CALL(cudaMemcpy(g_rays.rays, d_rays, rend*sizeof(Ray), cudaMemcpyDefault), s_rayReadTime);
	TIMED_CALL(cudaMemcpy(h_hits, d_hits, rend*sizeof(int), cudaMemcpyDefault), s_hitReadTime);
	...
	for(int i = 0; i < rend; ++i)
	{
		if(h_hits[i] >= 0)
		{
			g_rays.hits[g_rays.rays[i].id] = tbegin + h_hits[i];
		}
	}
...
void filterRays(const Box& box, int& rend)
{
	...
	for(int r = 0; r < rend; ++r)
	{
		...
		tmax = std::min(ray.tmax, std::min(tmax, std::max(tz1, tz2)));
...
void reshape(int w, int h)
{
	...
	delete[] g_rays.tmaxs;
	g_rays.tmaxs = new float[g_rays.count];

	delete[] g_rays.hits;
	g_rays.hits = new int[g_rays.count+1];

	g_paddingRay.id = g_rays.count;
	...
	cudaFreeHost(h_hits);
	cudaMallocHost(&h_hits, g_rays.count*sizeof(int));
...
void render(unsigned char* pixels)
{
	...
	std::cout << "    " << "rayRead:    " << std::setw(9) << s_rayReadTime << " ms" << std::endl;
